...
class Cluster {
 ...
 public:
  ...
  bool operator==(Cluster const& other) const {
    ...
    if (hits_.size() != other.hits_.size()) {
      return false;
    }
...
static std::vector<std::vector<Hit>> all_hits = {
...
BOOST_AUTO_TEST_CASE(write) {
  ...
  hdtree::Branch<std::vector<SpecialHit>> vector_special_hit_ds(
      "vector_special_hit");
  ...
  for (std::size_t i_entry{0}; i_entry < doubles.size(); i_entry++) {
    ...
    std::map<int, Cluster> map_clusters = {{2, Cluster(2, all_hits.at(0))},
